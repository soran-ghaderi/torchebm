...
`model_time` also accepts a callable `t -> t'` for clock schedules and reparametrisations; apply the same map at sampling time. With `ct="constant"`, `ct_multiplier=1` and `model_time="true"` the EqM objective is bit-identical to `FlowMatchingLoss(negate_velocity=True)`. `EqMEnergy` always evaluates the field at `t = 0`, so it suits time-invariant fields.
