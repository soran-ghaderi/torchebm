...
def test_energy_function():
    ...
    assert torch.allclose(energy, expected_energy), f"Expected energy {expected_energy}, but got {energy}"

    # Test gradient method
    grad = energy_fn.gradient(x)
    expected_grad = torch.tensor([[2.0, 4.0], [6.0, 8.0]])
    assert torch.allclose(grad, expected_grad), f"Expected gradient {expected_grad}, but got {grad}"
